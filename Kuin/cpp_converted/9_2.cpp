...
static void k_i();
static void k_j();
static void k_l(int64_t k_bh /*excpt*/);
static void k_p(type_(k_o) k_bk /*me*/);
static void k_q(k_o* k_bl);
static int64_t k_r(type_(k_o) k_bm /*me*/, type_(k_o) k_bn /*t*/);
static type_(k_o) k_s(type_(k_o) k_bo /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_br /*me*/);
static type_(k_o) k_u(type_(k_o) k_bt /*me*/, type_(Array_<uint8_t>) k_bu /*bin*/, int64_t* k_bv /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_by /*me*/);
static void k_w(type_(k_n) k_bz /*me*/);
using k_cb=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cc /*me*/, int64_t k_cd /*min*/, int64_t k_ce /*max*/);
using k_co=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cr /*me*/);
using k_ct=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cw /*me*/, double k_cx /*min*/, double k_cy /*max*/);
...
template<typename T_>static void k_ao(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/);
template<typename T_>static T_ k_at(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, int64_t k_fe /*start*/, int64_t k_ff /*len*/);
static void k_bg(type_(Array_<char16_t>) k_fg /*str*/);
template<typename T_>static T_ k_cu(T_ k_fh /*me_*/, type_(Array_<int64_t>) k_fi /*type*/, T_ k_fj /*n*/);
template<typename T_>static T_ k_cv(T_ k_fk /*me_*/, type_(Array_<int64_t>) k_fl /*type*/, int64_t k_fm /*n*/);
template<typename T_>static T_ k_dj(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/, int64_t k_fr /*n*/);
static char16_t k_en();
...
int64_t k_ap; // i
type_(Array_<char16_t>) k_as;
type_(Array_<char16_t>) k_aw;
type_(Array_<char16_t>) k_az;
type_(Array_<char16_t>) k_bc;
(k_am) = ((k_an)());
k_ao<type_(Array_<char16_t>)>((k_am), (nullptr));
(k_ap) = (0LL);
while((k_ap) < ((k_am)->Len())){
if((((k_ap) + (5LL)) <= ((k_am)->Len())) && (k_as=(k_at<type_(Array_<char16_t>)>((k_am), (nullptr), (k_ap), (5LL))),k_as->L==5&&k_as->B[0]==109&&k_as->B[1]==97&&k_as->B[2]==101&&k_as->B[3]==114&&k_as->B[4]==100)){
(k_ap) = ((k_ap) + (5LL));
}
else if((((k_ap) + (7LL)) <= ((k_am)->Len())) && (k_aw=(k_at<type_(Array_<char16_t>)>((k_am), (nullptr), (k_ap), (7LL))),k_aw->L==7&&k_aw->B[0]==114&&k_aw->B[1]==101&&k_aw->B[2]==109&&k_aw->B[3]==97&&k_aw->B[4]==101&&k_aw->B[5]==114&&k_aw->B[6]==100)){
(k_ap) = ((k_ap) + (7LL));
}
else if((((k_ap) + (5LL)) <= ((k_am)->Len())) && (k_az=(k_at<type_(Array_<char16_t>)>((k_am), (nullptr), (k_ap), (5LL))),k_az->L==5&&k_az->B[0]==101&&k_az->B[1]==115&&k_az->B[2]==97&&k_az->B[3]==114&&k_az->B[4]==101)){
(k_ap) = ((k_ap) + (5LL));
}
else if((((k_ap) + (6LL)) <= ((k_am)->Len())) && (k_bc=(k_at<type_(Array_<char16_t>)>((k_am), (nullptr), (k_ap), (6LL))),k_bc->L==6&&k_bc->B[0]==114&&k_bc->B[1]==101&&k_bc->B[2]==115&&k_bc->B[3]==97&&k_bc->B[4]==114&&k_bc->B[5]==101)){
(k_ap) = ((k_ap) + (6LL));
}
else{
(k_bg)((newArrayBin_<char16_t>(3,"\x4E\x00\x4F\x00\x0A\x00")));
return;
}
}
(k_bg)((newArrayBin_<char16_t>(4,"\x59\x00\x45\x00\x53\x00\x0A\x00")));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_bp;
type_(k_o) k_bq;
(k_bp) = (k_bq = new_(k_o)(), k_p(k_bq), k_bq);
return as_<k_o>(classTable_, (k_bp), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_br /*me*/){
type_(Array_<uint8_t>) k_bs;
(k_bs) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bs;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bt /*me*/, type_(Array_<uint8_t>) k_bu /*bin*/, int64_t* k_bv /*idx*/){
...
(k_cj) = (static_cast<uint64_t>(((k_ce) - (k_cd)) + (1LL)));
(k_ck) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cj)) % (k_cj)));
(k_cl) = (0x0000000000000000ULL);
if((k_ck) == (0x0000000000000000ULL)){
(k_cl) = ((reinterpret_cast<k_co>(classTable_[(k_cc)->Y + 9]))((k_cc)));
}
else{
do{
(k_cl) = ((reinterpret_cast<k_co>(classTable_[(k_cc)->Y + 9]))((k_cc)));
...
return (static_cast<int64_t>((k_cl) % (k_cj))) + (k_cd);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cr /*me*/){
uint32_t k_cs; // t
(k_cs) = ((reinterpret_cast<k_ct>(classTable_[(k_cr)->Y + 12]))((k_cr)));
return k_cu<uint64_t>((k_cv<uint64_t>((static_cast<uint64_t>(k_cs)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_ct>(classTable_[(k_cr)->Y + 12]))((k_cr)))));
}
// rndFloat
static double k_z(type_(k_n) k_cw /*me*/, double k_cx /*min*/, double k_cy /*max*/){
if((k_cx) >= (k_cy)){
...
return (((static_cast<double>((reinterpret_cast<k_co>(classTable_[(k_cw)->Y + 9]))((k_cw)))) / (1.844674407370955e+19)) * ((k_cy) - (k_cx))) + (k_cx);
}
// setSeed
static void k_aa(type_(k_n) k_dd /*me*/, uint32_t k_de /*seed*/){
...
static uint32_t k_ab(type_(k_n) k_dk /*me*/){
uint32_t k_dl; // t
(k_dl) = (k_ai<uint32_t>(((k_dk)->k_df), (nullptr), (k_cv<uint32_t>(((k_dk)->k_df), (nullptr), (11LL)))));
((k_dk)->k_df) = ((k_dk)->k_dg);
((k_dk)->k_dg) = ((k_dk)->k_dh);
((k_dk)->k_dh) = ((k_dk)->k_di);
...
// inputStr
static type_(Array_<char16_t>) k_an(){
type_(Array_<char16_t>) k_eh; // buf
int64_t k_ei; // ptr
char16_t k_em; // c
(k_eh) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_ei) = (0LL);
while(true){
(k_em) = ((k_en)());
if((k_em) == (u'\uFFFF')){
...
if((k_ei) == ((k_eh)->Len())){
(k_eh) = ((k_eh)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_eh)->At(k_ei)) = (k_em);
(k_ei) = ((k_ei) + (1LL));
}
return k_at<type_(Array_<char16_t>)>((k_eh), (nullptr), (0LL), (k_ei));
}
// _reverse
template<typename T_>static void k_ao(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/){
reverse_(k_fa);
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fg->B),stdout);
#else
std::u16string s_=k_fg->B;
...
// _shr
template<typename T_>static T_ k_dj(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/, int64_t k_fr /*n*/){
if(k_fr<0||k_fr>=sizeof(k_fp)*8)throw 0xe9170006;
return k_fp>>k_fr;
}
