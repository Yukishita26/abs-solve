...
static void k_aa(type_(k_n) k_di /*me*/, uint32_t k_dj /*seed*/);
static uint32_t k_ab(type_(k_n) k_dp /*me*/);
static void k_ac(k_n* k_dr);
static type_(k_n) k_ad(type_(k_n) k_ds);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dv);
static type_(k_n) k_af(type_(k_n) k_eb, type_(Array_<uint8_t>) k_ec, int64_t* k_ed);
static type_(k_n) k_ag(type_(k_n) k_eg /*me2*/, uint32_t k_eh /*seed*/);
using k_ei=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_ej /*me_*/, type_(Array_<int64_t>) k_ek /*type*/, T_ k_el /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_bk(type_(Array_<char16_t>) k_et /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bl(T_ k_eu /*me_*/, type_(Array_<int64_t>) k_ev /*type*/);
template<typename T_>static T_ k_cz(T_ k_ew /*me_*/, type_(Array_<int64_t>) k_ex /*type*/, T_ k_ey /*n*/);
template<typename T_>static T_ k_da(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, int64_t k_fb /*n*/);
template<typename T_>static T_ k_do(T_ k_fe /*me_*/, type_(Array_<int64_t>) k_ff /*type*/, int64_t k_fg /*n*/);
static type_(Array_<char16_t>) k_en();
static int64_t k_eq(type_(Array_<char16_t>) k_gd /*me_*/, bool* k_ge /*success*/);
static char16_t k_fp();
template<typename T_>static T_ k_gc(T_ k_gt /*me_*/, type_(Array_<int64_t>) k_gu /*type*/, int64_t k_gv /*start*/, int64_t k_gw /*len*/);
static char16_t k_gj();
...
k_n::k_n(): k_o(), k_dk(), k_dl(), k_dm(), k_dn(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_aq) = ((k_an)());
(k_ar) = (0LL);
for(k_av = (0LL), k_aw = (k_am); k_av <= k_aw; k_av += (1LL)){
for(k_ba = (0LL), k_bb = (k_ao); k_ba <= k_bb; k_ba += (1LL)){
for(k_bf = (0LL), k_bg = (k_ap); k_bf <= k_bg; k_bf += (1LL)){
if(((((500LL) * (k_av)) + ((100LL) * (k_ba))) + ((50LL) * (k_bf))) == (k_aq)){
(k_ar) = ((k_ar) + (1LL));
}
}
}
}
(k_bk)((((newArrayBin_<char16_t>(0,""))->Cat(k_bl<int64_t>((k_ar), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return (static_cast<int64_t>((k_cq) % (k_co))) + (k_ci);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cw /*me*/){
uint32_t k_cx; // t
...
// setSeed
static void k_aa(type_(k_n) k_di /*me*/, uint32_t k_dj /*seed*/){
((k_di)->k_dk) = (0x075BCD15U);
((k_di)->k_dl) = (0x159A55E5U);
((k_di)->k_dm) = ((0x1F123BB5U) * (k_dj));
((k_di)->k_dn) = ((0x05491333U) * (k_do<uint32_t>(((k_di)->k_dm), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dp /*me*/){
uint32_t k_dq; // t
(k_dq) = (k_ai<uint32_t>(((k_dp)->k_dk), (nullptr), (k_da<uint32_t>(((k_dp)->k_dk), (nullptr), (11LL)))));
((k_dp)->k_dk) = ((k_dp)->k_dl);
((k_dp)->k_dl) = ((k_dp)->k_dm);
((k_dp)->k_dm) = ((k_dp)->k_dn);
((k_dp)->k_dn) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dp)->k_dn), (nullptr), (k_do<uint32_t>(((k_dp)->k_dn), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dq), (nullptr), (k_do<uint32_t>((k_dq), (nullptr), (8LL)))))));
return (k_dp)->k_dn;
}
// _dtor
static void k_ac(k_n* k_dr){
...
((k_dt)->k_dm) = ((k_ds)->k_dm);
((k_dt)->k_dn) = ((k_ds)->k_dn);
return as_<k_n>(classTable_, (k_dt), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dv){
type_(Array_<uint8_t>) k_dw;
(k_dw) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dw) = ((k_dw)->Cat(toBin_<uint32_t>()(((k_dv)->k_dk))));
(k_dw) = ((k_dw)->Cat(toBin_<uint32_t>()(((k_dv)->k_dl))));
(k_dw) = ((k_dw)->Cat(toBin_<uint32_t>()(((k_dv)->k_dm))));
(k_dw) = ((k_dw)->Cat(toBin_<uint32_t>()(((k_dv)->k_dn))));
return k_dw;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_eb, type_(Array_<uint8_t>) k_ec, int64_t* k_ed){
type_(k_n) k_ee;
type_(k_n) k_ef;
(k_ee) = (k_ef = new_(k_n)(), k_w(k_ef), k_ef);
((k_ee)->k_dk) = (fromBin_<uint32_t>()((k_ec), (*k_ed)));
((k_ee)->k_dl) = (fromBin_<uint32_t>()((k_ec), (*k_ed)));
((k_ee)->k_dm) = (fromBin_<uint32_t>()((k_ec), (*k_ed)));
((k_ee)->k_dn) = (fromBin_<uint32_t>()((k_ec), (*k_ed)));
return k_ee;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_eg /*me2*/, uint32_t k_eh /*seed*/){
(reinterpret_cast<k_ei>(classTable_[(k_eg)->Y + 11]))((k_eg), (k_eh));
return k_eg;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_ej /*me_*/, type_(Array_<int64_t>) k_ek /*type*/, T_ k_el /*n*/){
...
// inputStr
static type_(Array_<char16_t>) k_en(){
type_(Array_<char16_t>) k_fj; // buf
int64_t k_fk; // ptr
char16_t k_fo; // c
(k_fj) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fk) = (0LL);
while(true){
(k_fo) = ((k_fp)());
if((k_fo) == (u'\uFFFF')){
...
if((k_fk) == ((k_fj)->Len())){
(k_fj) = ((k_fj)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_fj)->At(k_fk)) = (k_fo);
(k_fk) = ((k_fk) + (1LL));
}
return k_gc<type_(Array_<char16_t>)>((k_fj), (nullptr), (0LL), (k_fk));
}
// _toInt
static int64_t k_eq(type_(Array_<char16_t>) k_gd /*me_*/, bool* k_ge /*success*/){
std::u16string u_=k_gd->B;
...
while(true){
(k_gi) = ((k_gj)());
for(k_gn = (0LL), k_go = (((k_al)->Len()) - (1LL)); k_gn <= k_go; k_gn += (1LL)){
if(((k_gi) == ((k_al)->At(k_gn))) || (((k_gi) == (u'\u000D')) && (((k_al)->At(k_gn)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gi) == (u'\u000D')){
