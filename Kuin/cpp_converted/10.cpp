...
static void k_i();
static void k_j();
static void k_l(int64_t k_bg /*excpt*/);
static void k_p(type_(k_o) k_bj /*me*/);
static void k_q(k_o* k_bk);
static int64_t k_r(type_(k_o) k_bl /*me*/, type_(k_o) k_bm /*t*/);
static type_(k_o) k_s(type_(k_o) k_bn /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bq /*me*/);
static type_(k_o) k_u(type_(k_o) k_bs /*me*/, type_(Array_<uint8_t>) k_bt /*bin*/, int64_t* k_bu /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bx /*me*/);
static void k_w(type_(k_n) k_by /*me*/);
using k_ca=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cb /*me*/, int64_t k_cc /*min*/, int64_t k_cd /*max*/);
using k_cn=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cq /*me*/);
using k_cs=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cv /*me*/, double k_cw /*min*/, double k_cx /*max*/);
static void k_aa(type_(k_n) k_dc /*me*/, uint32_t k_dd /*seed*/);
static uint32_t k_ab(type_(k_n) k_dj /*me*/);
static void k_ac(k_n* k_dl);
static type_(k_n) k_ad(type_(k_n) k_dm);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dp);
static type_(k_n) k_af(type_(k_n) k_dv, type_(Array_<uint8_t>) k_dw, int64_t* k_dx);
static type_(k_n) k_ag(type_(k_n) k_ea /*me2*/, uint32_t k_eb /*seed*/);
using k_ec=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_ed /*me_*/, type_(Array_<int64_t>) k_ee /*type*/, T_ k_ef /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static int64_t k_bc(int64_t k_en /*me_*/);
static void k_bf(type_(Array_<char16_t>) k_eo /*str*/);
template<typename T_>static T_ k_ct(T_ k_ep /*me_*/, type_(Array_<int64_t>) k_eq /*type*/, T_ k_er /*n*/);
template<typename T_>static T_ k_cu(T_ k_es /*me_*/, type_(Array_<int64_t>) k_et /*type*/, int64_t k_eu /*n*/);
template<typename T_>static T_ k_di(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/, int64_t k_ez /*n*/);
static type_(Array_<char16_t>) k_eh();
static int64_t k_ek(type_(Array_<char16_t>) k_fw /*me_*/, bool* k_fx /*success*/);
static char16_t k_fi();
template<typename T_>static T_ k_fv(T_ k_gm /*me_*/, type_(Array_<int64_t>) k_gn /*type*/, int64_t k_go /*start*/, int64_t k_gp /*len*/);
static char16_t k_gc();
...
k_n::k_n(): k_o(), k_de(), k_df(), k_dg(), k_dh(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_aq) = (0LL);
(k_ar) = (true);
for(k_av = (0LL), k_aw = ((k_am) - (1LL)); k_av <= k_aw; k_av += (1LL)){
(k_ax) = ((k_an)());
(k_ay) = ((k_an)());
(k_az) = ((k_an)());
(k_ba) = ((k_ax) - (k_ao));
(k_bb) = (((k_bc)(((k_ay) - (k_ap)))) + ((k_bc)(((k_az) - (k_aq)))));
if(((k_bb) > (k_ba)) || (((k_bb) % (2LL)) != ((k_ba) % (2LL)))){
(k_ar) = (false);
}
(k_ao) = (k_ax);
(k_ap) = (k_ay);
...
(k_bf)((((k_ar) ? (newArrayBin_<char16_t>(3,"\x59\x00\x65\x00\x73\x00")) : (newArrayBin_<char16_t>(2,"\x4E\x00\x6F\x00")))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bo), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bq /*me*/){
type_(Array_<uint8_t>) k_br;
(k_br) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_br;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bs /*me*/, type_(Array_<uint8_t>) k_bt /*bin*/, int64_t* k_bu /*idx*/){
type_(k_o) k_bv;
type_(k_o) k_bw;
(k_bv) = (k_bw = new_(k_o)(), k_p(k_bw), k_bw);
return k_bv;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bx /*me*/){
...
(k_ci) = (static_cast<uint64_t>(((k_cd) - (k_cc)) + (1LL)));
(k_cj) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_ci)) % (k_ci)));
(k_ck) = (0x0000000000000000ULL);
if((k_cj) == (0x0000000000000000ULL)){
(k_ck) = ((reinterpret_cast<k_cn>(classTable_[(k_cb)->Y + 9]))((k_cb)));
}
else{
do{
(k_ck) = ((reinterpret_cast<k_cn>(classTable_[(k_cb)->Y + 9]))((k_cb)));
...
return (static_cast<int64_t>((k_ck) % (k_ci))) + (k_cc);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cq /*me*/){
uint32_t k_cr; // t
(k_cr) = ((reinterpret_cast<k_cs>(classTable_[(k_cq)->Y + 12]))((k_cq)));
return k_ct<uint64_t>((k_cu<uint64_t>((static_cast<uint64_t>(k_cr)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cs>(classTable_[(k_cq)->Y + 12]))((k_cq)))));
}
// rndFloat
static double k_z(type_(k_n) k_cv /*me*/, double k_cw /*min*/, double k_cx /*max*/){
if((k_cw) >= (k_cx)){
...
return (((static_cast<double>((reinterpret_cast<k_cn>(classTable_[(k_cv)->Y + 9]))((k_cv)))) / (1.844674407370955e+19)) * ((k_cx) - (k_cw))) + (k_cw);
}
// setSeed
static void k_aa(type_(k_n) k_dc /*me*/, uint32_t k_dd /*seed*/){
((k_dc)->k_de) = (0x075BCD15U);
((k_dc)->k_df) = (0x159A55E5U);
((k_dc)->k_dg) = ((0x1F123BB5U) * (k_dd));
((k_dc)->k_dh) = ((0x05491333U) * (k_di<uint32_t>(((k_dc)->k_dg), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dj /*me*/){
uint32_t k_dk; // t
(k_dk) = (k_ai<uint32_t>(((k_dj)->k_de), (nullptr), (k_cu<uint32_t>(((k_dj)->k_de), (nullptr), (11LL)))));
((k_dj)->k_de) = ((k_dj)->k_df);
((k_dj)->k_df) = ((k_dj)->k_dg);
((k_dj)->k_dg) = ((k_dj)->k_dh);
((k_dj)->k_dh) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dj)->k_dh), (nullptr), (k_di<uint32_t>(((k_dj)->k_dh), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dk), (nullptr), (k_di<uint32_t>((k_dk), (nullptr), (8LL)))))));
return (k_dj)->k_dh;
}
// _dtor
static void k_ac(k_n* k_dl){
...
type_(k_n) k_dn;
type_(k_n) k_do;
(k_dn) = (k_do = new_(k_n)(), k_w(k_do), k_do);
((k_dn)->k_de) = ((k_dm)->k_de);
((k_dn)->k_df) = ((k_dm)->k_df);
((k_dn)->k_dg) = ((k_dm)->k_dg);
((k_dn)->k_dh) = ((k_dm)->k_dh);
return as_<k_n>(classTable_, (k_dn), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dp){
type_(Array_<uint8_t>) k_dq;
(k_dq) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dq) = ((k_dq)->Cat(toBin_<uint32_t>()(((k_dp)->k_de))));
(k_dq) = ((k_dq)->Cat(toBin_<uint32_t>()(((k_dp)->k_df))));
(k_dq) = ((k_dq)->Cat(toBin_<uint32_t>()(((k_dp)->k_dg))));
(k_dq) = ((k_dq)->Cat(toBin_<uint32_t>()(((k_dp)->k_dh))));
return k_dq;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dv, type_(Array_<uint8_t>) k_dw, int64_t* k_dx){
type_(k_n) k_dy;
type_(k_n) k_dz;
(k_dy) = (k_dz = new_(k_n)(), k_w(k_dz), k_dz);
((k_dy)->k_de) = (fromBin_<uint32_t>()((k_dw), (*k_dx)));
((k_dy)->k_df) = (fromBin_<uint32_t>()((k_dw), (*k_dx)));
((k_dy)->k_dg) = (fromBin_<uint32_t>()((k_dw), (*k_dx)));
((k_dy)->k_dh) = (fromBin_<uint32_t>()((k_dw), (*k_dx)));
return k_dy;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_ea /*me2*/, uint32_t k_eb /*seed*/){
(reinterpret_cast<k_ec>(classTable_[(k_ea)->Y + 11]))((k_ea), (k_eb));
return k_ea;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_ed /*me_*/, type_(Array_<int64_t>) k_ee /*type*/, T_ k_ef /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_eo->B),stdout);
#else
std::u16string s_=k_eo->B;
...
// _shl
template<typename T_>static T_ k_cu(T_ k_es /*me_*/, type_(Array_<int64_t>) k_et /*type*/, int64_t k_eu /*n*/){
if(k_eu<0||k_eu>=sizeof(k_es)*8)throw 0xe9170006;
return k_es<<k_eu;
}
// _shr
template<typename T_>static T_ k_di(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/, int64_t k_ez /*n*/){
if(k_ez<0||k_ez>=sizeof(k_ex)*8)throw 0xe9170006;
return k_ex>>k_ez;
}
// inputStr
static type_(Array_<char16_t>) k_eh(){
...
if((k_fd) == ((k_fc)->Len())){
(k_fc) = ((k_fc)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_fc)->At(k_fd)) = (k_fh);
(k_fd) = ((k_fd) + (1LL));
}
...
while(true){
(k_gb) = ((k_gc)());
for(k_gg = (0LL), k_gh = (((k_al)->Len()) - (1LL)); k_gg <= k_gh; k_gg += (1LL)){
if(((k_gb) == ((k_al)->At(k_gg))) || (((k_gb) == (u'\u000D')) && (((k_al)->At(k_gg)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gb) == (u'\u000D')){
