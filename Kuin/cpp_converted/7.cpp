...
static void k_i();
static void k_j();
static void k_l(int64_t k_ax /*excpt*/);
static void k_p(type_(k_o) k_ba /*me*/);
static void k_q(k_o* k_bb);
static int64_t k_r(type_(k_o) k_bc /*me*/, type_(k_o) k_bd /*t*/);
static type_(k_o) k_s(type_(k_o) k_be /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bh /*me*/);
static type_(k_o) k_u(type_(k_o) k_bj /*me*/, type_(Array_<uint8_t>) k_bk /*bin*/, int64_t* k_bl /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bo /*me*/);
static void k_w(type_(k_n) k_bp /*me*/);
using k_br=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bs /*me*/, int64_t k_bt /*min*/, int64_t k_bu /*max*/);
using k_ce=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ch /*me*/);
using k_cj=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cm /*me*/, double k_cn /*min*/, double k_co /*max*/);
static void k_aa(type_(k_n) k_ct /*me*/, uint32_t k_cu /*seed*/);
static uint32_t k_ab(type_(k_n) k_da /*me*/);
static void k_ac(k_n* k_dc);
static type_(k_n) k_ad(type_(k_n) k_dd);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dg);
static type_(k_n) k_af(type_(k_n) k_dm, type_(Array_<uint8_t>) k_dn, int64_t* k_do);
static type_(k_n) k_ag(type_(k_n) k_dr /*me2*/, uint32_t k_ds /*seed*/);
using k_dt=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_du /*me_*/, type_(Array_<int64_t>) k_dv /*type*/, T_ k_dw /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename K_,typename V_>static void k_au(T_ k_ee /*me_*/, type_(Array_<int64_t>) k_ef /*type*/, K_ k_eg /*key*/, V_ k_eh /*item*/);
static void k_av(type_(Array_<char16_t>) k_ei /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_aw(T_ k_ej /*me_*/, type_(Array_<int64_t>) k_ek /*type*/);
template<typename T_>static T_ k_ck(T_ k_el /*me_*/, type_(Array_<int64_t>) k_em /*type*/, T_ k_en /*n*/);
template<typename T_>static T_ k_cl(T_ k_eo /*me_*/, type_(Array_<int64_t>) k_ep /*type*/, int64_t k_eq /*n*/);
template<typename T_>static T_ k_cz(T_ k_et /*me_*/, type_(Array_<int64_t>) k_eu /*type*/, int64_t k_ev /*n*/);
static type_(Array_<char16_t>) k_dy();
static int64_t k_eb(type_(Array_<char16_t>) k_fs /*me_*/, bool* k_ft /*success*/);
...
k_n::k_n(): k_o(), k_cv(), k_cw(), k_cx(), k_cy(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
static void k_g(){
int64_t k_am; // n
type_(Dict_<int64_t, int64_t>) k_ao; // dic
int64_t k_as;
int64_t k_at;
(k_am) = ((k_an)());
(k_ao) = (new_(Dict_<int64_t, int64_t>)());
for(k_as = (0LL), k_at = ((k_am) - (1LL)); k_as <= k_at; k_as += (1LL)){
k_au<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_ao), (nullptr), ((k_an)()), (1LL));
}
(k_av)((((newArrayBin_<char16_t>(0,""))->Cat(k_aw<int64_t>(((k_ao)->Len()), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bf), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bh /*me*/){
type_(Array_<uint8_t>) k_bi;
(k_bi) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bi;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bj /*me*/, type_(Array_<uint8_t>) k_bk /*bin*/, int64_t* k_bl /*idx*/){
type_(k_o) k_bm;
type_(k_o) k_bn;
(k_bm) = (k_bn = new_(k_o)(), k_p(k_bn), k_bn);
return k_bm;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bo /*me*/){
...
return (static_cast<int64_t>((k_cb) % (k_bz))) + (k_bt);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ch /*me*/){
uint32_t k_ci; // t
...
// setSeed
static void k_aa(type_(k_n) k_ct /*me*/, uint32_t k_cu /*seed*/){
((k_ct)->k_cv) = (0x075BCD15U);
((k_ct)->k_cw) = (0x159A55E5U);
((k_ct)->k_cx) = ((0x1F123BB5U) * (k_cu));
((k_ct)->k_cy) = ((0x05491333U) * (k_cz<uint32_t>(((k_ct)->k_cx), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_da /*me*/){
uint32_t k_db; // t
(k_db) = (k_ai<uint32_t>(((k_da)->k_cv), (nullptr), (k_cl<uint32_t>(((k_da)->k_cv), (nullptr), (11LL)))));
((k_da)->k_cv) = ((k_da)->k_cw);
((k_da)->k_cw) = ((k_da)->k_cx);
((k_da)->k_cx) = ((k_da)->k_cy);
((k_da)->k_cy) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_da)->k_cy), (nullptr), (k_cz<uint32_t>(((k_da)->k_cy), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_db), (nullptr), (k_cz<uint32_t>((k_db), (nullptr), (8LL)))))));
return (k_da)->k_cy;
}
// _dtor
static void k_ac(k_n* k_dc){
...
type_(k_n) k_de;
type_(k_n) k_df;
(k_de) = (k_df = new_(k_n)(), k_w(k_df), k_df);
((k_de)->k_cv) = ((k_dd)->k_cv);
((k_de)->k_cw) = ((k_dd)->k_cw);
((k_de)->k_cx) = ((k_dd)->k_cx);
((k_de)->k_cy) = ((k_dd)->k_cy);
return as_<k_n>(classTable_, (k_de), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dg){
type_(Array_<uint8_t>) k_dh;
(k_dh) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dh) = ((k_dh)->Cat(toBin_<uint32_t>()(((k_dg)->k_cv))));
(k_dh) = ((k_dh)->Cat(toBin_<uint32_t>()(((k_dg)->k_cw))));
(k_dh) = ((k_dh)->Cat(toBin_<uint32_t>()(((k_dg)->k_cx))));
(k_dh) = ((k_dh)->Cat(toBin_<uint32_t>()(((k_dg)->k_cy))));
return k_dh;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dm, type_(Array_<uint8_t>) k_dn, int64_t* k_do){
...
((k_dp)->k_cv) = (fromBin_<uint32_t>()((k_dn), (*k_do)));
((k_dp)->k_cw) = (fromBin_<uint32_t>()((k_dn), (*k_do)));
((k_dp)->k_cx) = (fromBin_<uint32_t>()((k_dn), (*k_do)));
((k_dp)->k_cy) = (fromBin_<uint32_t>()((k_dn), (*k_do)));
return k_dp;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_dr /*me2*/, uint32_t k_ds /*seed*/){
(reinterpret_cast<k_dt>(classTable_[(k_dr)->Y + 11]))((k_dr), (k_ds));
return k_dr;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_du /*me_*/, type_(Array_<int64_t>) k_dv /*type*/, T_ k_dw /*n*/){
...
// _shr
template<typename T_>static T_ k_cz(T_ k_et /*me_*/, type_(Array_<int64_t>) k_eu /*type*/, int64_t k_ev /*n*/){
if(k_ev<0||k_ev>=sizeof(k_et)*8)throw 0xe9170006;
return k_et>>k_ev;
}
// inputStr
static type_(Array_<char16_t>) k_dy(){
...
if((k_ez) == ((k_ey)->Len())){
(k_ey) = ((k_ey)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_ey)->At(k_ez)) = (k_fd);
(k_ez) = ((k_ez) + (1LL));
}
