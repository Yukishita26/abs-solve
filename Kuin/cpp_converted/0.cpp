...
static void k_i();
static void k_j();
static void k_l(int64_t k_au /*excpt*/);
static void k_p(type_(k_o) k_ax /*me*/);
static void k_q(k_o* k_ay);
static int64_t k_r(type_(k_o) k_az /*me*/, type_(k_o) k_ba /*t*/);
static type_(k_o) k_s(type_(k_o) k_bb /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_be /*me*/);
static type_(k_o) k_u(type_(k_o) k_bg /*me*/, type_(Array_<uint8_t>) k_bh /*bin*/, int64_t* k_bi /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bl /*me*/);
static void k_w(type_(k_n) k_bm /*me*/);
using k_bo=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bp /*me*/, int64_t k_bq /*min*/, int64_t k_br /*max*/);
using k_cb=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ce /*me*/);
using k_cg=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cj /*me*/, double k_ck /*min*/, double k_cl /*max*/);
static void k_aa(type_(k_n) k_cq /*me*/, uint32_t k_cr /*seed*/);
static uint32_t k_ab(type_(k_n) k_cx /*me*/);
static void k_ac(k_n* k_cz);
static type_(k_n) k_ad(type_(k_n) k_da);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dd);
static type_(k_n) k_af(type_(k_n) k_dj, type_(Array_<uint8_t>) k_dk, int64_t* k_dl);
static type_(k_n) k_ag(type_(k_n) k_do /*me2*/, uint32_t k_dp /*seed*/);
using k_dq=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dr /*me_*/, type_(Array_<int64_t>) k_ds /*type*/, T_ k_dt /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static type_(Array_<char16_t>) k_ar();
static void k_as(type_(Array_<char16_t>) k_eu /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_at(T_ k_ev /*me_*/, type_(Array_<int64_t>) k_ew /*type*/);
template<typename T_>static T_ k_ch(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/, T_ k_ez /*n*/);
template<typename T_>static T_ k_ci(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/, int64_t k_fc /*n*/);
template<typename T_>static T_ k_cw(T_ k_ff /*me_*/, type_(Array_<int64_t>) k_fg /*type*/, int64_t k_fh /*n*/);
static int64_t k_dx(type_(Array_<char16_t>) k_fk /*me_*/, bool* k_fl /*success*/);
static char16_t k_eg();
template<typename T_>static T_ k_et(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/, int64_t k_gc /*start*/, int64_t k_gd /*len*/);
static char16_t k_fq();
...
k_n::k_n(): k_o(), k_cs(), k_ct(), k_cu(), k_cv(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_ap) = ((k_an)());
(k_aq) = ((k_ar)());
(k_as)((((newArrayBin_<char16_t>(0,""))->Cat(k_at<int64_t>((((k_am) + (k_ao)) + (k_ap)), (nullptr))))->Cat(((newArrayBin_<char16_t>(1,"\x20\x00"))->Cat(k_at<type_(Array_<char16_t>)>((k_aq), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00")))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_bc;
type_(k_o) k_bd;
(k_bc) = (k_bd = new_(k_o)(), k_p(k_bd), k_bd);
return as_<k_o>(classTable_, (k_bc), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_be /*me*/){
type_(Array_<uint8_t>) k_bf;
(k_bf) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bf;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bg /*me*/, type_(Array_<uint8_t>) k_bh /*bin*/, int64_t* k_bi /*idx*/){
type_(k_o) k_bj;
type_(k_o) k_bk;
(k_bj) = (k_bk = new_(k_o)(), k_p(k_bk), k_bk);
return k_bj;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bl /*me*/){
...
(k_bw) = (static_cast<uint64_t>(((k_br) - (k_bq)) + (1LL)));
(k_bx) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_bw)) % (k_bw)));
(k_by) = (0x0000000000000000ULL);
if((k_bx) == (0x0000000000000000ULL)){
(k_by) = ((reinterpret_cast<k_cb>(classTable_[(k_bp)->Y + 9]))((k_bp)));
}
else{
do{
(k_by) = ((reinterpret_cast<k_cb>(classTable_[(k_bp)->Y + 9]))((k_bp)));
...
return (static_cast<int64_t>((k_by) % (k_bw))) + (k_bq);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ce /*me*/){
uint32_t k_cf; // t
(k_cf) = ((reinterpret_cast<k_cg>(classTable_[(k_ce)->Y + 12]))((k_ce)));
return k_ch<uint64_t>((k_ci<uint64_t>((static_cast<uint64_t>(k_cf)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cg>(classTable_[(k_ce)->Y + 12]))((k_ce)))));
}
// rndFloat
static double k_z(type_(k_n) k_cj /*me*/, double k_ck /*min*/, double k_cl /*max*/){
if((k_ck) >= (k_cl)){
...
return (((static_cast<double>((reinterpret_cast<k_cb>(classTable_[(k_cj)->Y + 9]))((k_cj)))) / (1.844674407370955e+19)) * ((k_cl) - (k_ck))) + (k_ck);
}
// setSeed
static void k_aa(type_(k_n) k_cq /*me*/, uint32_t k_cr /*seed*/){
((k_cq)->k_cs) = (0x075BCD15U);
((k_cq)->k_ct) = (0x159A55E5U);
((k_cq)->k_cu) = ((0x1F123BB5U) * (k_cr));
((k_cq)->k_cv) = ((0x05491333U) * (k_cw<uint32_t>(((k_cq)->k_cu), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_cx /*me*/){
uint32_t k_cy; // t
(k_cy) = (k_ai<uint32_t>(((k_cx)->k_cs), (nullptr), (k_ci<uint32_t>(((k_cx)->k_cs), (nullptr), (11LL)))));
((k_cx)->k_cs) = ((k_cx)->k_ct);
((k_cx)->k_ct) = ((k_cx)->k_cu);
((k_cx)->k_cu) = ((k_cx)->k_cv);
((k_cx)->k_cv) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_cx)->k_cv), (nullptr), (k_cw<uint32_t>(((k_cx)->k_cv), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_cy), (nullptr), (k_cw<uint32_t>((k_cy), (nullptr), (8LL)))))));
return (k_cx)->k_cv;
}
// _dtor
static void k_ac(k_n* k_cz){
...
type_(k_n) k_db;
type_(k_n) k_dc;
(k_db) = (k_dc = new_(k_n)(), k_w(k_dc), k_dc);
((k_db)->k_cs) = ((k_da)->k_cs);
((k_db)->k_ct) = ((k_da)->k_ct);
((k_db)->k_cu) = ((k_da)->k_cu);
((k_db)->k_cv) = ((k_da)->k_cv);
return as_<k_n>(classTable_, (k_db), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dd){
type_(Array_<uint8_t>) k_de;
(k_de) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_de) = ((k_de)->Cat(toBin_<uint32_t>()(((k_dd)->k_cs))));
(k_de) = ((k_de)->Cat(toBin_<uint32_t>()(((k_dd)->k_ct))));
(k_de) = ((k_de)->Cat(toBin_<uint32_t>()(((k_dd)->k_cu))));
(k_de) = ((k_de)->Cat(toBin_<uint32_t>()(((k_dd)->k_cv))));
return k_de;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dj, type_(Array_<uint8_t>) k_dk, int64_t* k_dl){
type_(k_n) k_dm;
type_(k_n) k_dn;
(k_dm) = (k_dn = new_(k_n)(), k_w(k_dn), k_dn);
((k_dm)->k_cs) = (fromBin_<uint32_t>()((k_dk), (*k_dl)));
((k_dm)->k_ct) = (fromBin_<uint32_t>()((k_dk), (*k_dl)));
((k_dm)->k_cu) = (fromBin_<uint32_t>()((k_dk), (*k_dl)));
((k_dm)->k_cv) = (fromBin_<uint32_t>()((k_dk), (*k_dl)));
return k_dm;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_do /*me2*/, uint32_t k_dp /*seed*/){
(reinterpret_cast<k_dq>(classTable_[(k_do)->Y + 11]))((k_do), (k_dp));
return k_do;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dr /*me_*/, type_(Array_<int64_t>) k_ds /*type*/, T_ k_dt /*n*/){
...
// inputStr
static type_(Array_<char16_t>) k_ar(){
type_(Array_<char16_t>) k_ea; // buf
int64_t k_eb; // ptr
char16_t k_ef; // c
(k_ea) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_eb) = (0LL);
while(true){
(k_ef) = ((k_eg)());
if((k_ef) == (u'\uFFFF')){
...
if((k_eb) == ((k_ea)->Len())){
(k_ea) = ((k_ea)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_ea)->At(k_eb)) = (k_ef);
(k_eb) = ((k_eb) + (1LL));
}
return k_et<type_(Array_<char16_t>)>((k_ea), (nullptr), (0LL), (k_eb));
}
// print
static void k_as(type_(Array_<char16_t>) k_eu /*str*/){
if(k_eu==nullptr){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_eu->B),stdout);
#else
std::u16string s_=k_eu->B;
...
// _shl
template<typename T_>static T_ k_ci(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/, int64_t k_fc /*n*/){
if(k_fc<0||k_fc>=sizeof(k_fa)*8)throw 0xe9170006;
return k_fa<<k_fc;
}
// _shr
template<typename T_>static T_ k_cw(T_ k_ff /*me_*/, type_(Array_<int64_t>) k_fg /*type*/, int64_t k_fh /*n*/){
...
while(true){
(k_fp) = ((k_fq)());
for(k_fu = (0LL), k_fv = (((k_al)->Len()) - (1LL)); k_fu <= k_fv; k_fu += (1LL)){
if(((k_fp) == ((k_al)->At(k_fu))) || (((k_fp) == (u'\u000D')) && (((k_al)->At(k_fu)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_fp) == (u'\u000D')){
