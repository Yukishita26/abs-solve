...
static void k_i();
static void k_j();
static void k_l(int64_t k_aq /*excpt*/);
static void k_p(type_(k_o) k_at /*me*/);
static void k_q(k_o* k_au);
static int64_t k_r(type_(k_o) k_av /*me*/, type_(k_o) k_aw /*t*/);
static type_(k_o) k_s(type_(k_o) k_ax /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ba /*me*/);
static type_(k_o) k_u(type_(k_o) k_bc /*me*/, type_(Array_<uint8_t>) k_bd /*bin*/, int64_t* k_be /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bh /*me*/);
static void k_w(type_(k_n) k_bi /*me*/);
using k_bk=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bl /*me*/, int64_t k_bm /*min*/, int64_t k_bn /*max*/);
using k_bx=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ca /*me*/);
using k_cc=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cf /*me*/, double k_cg /*min*/, double k_ch /*max*/);
static void k_aa(type_(k_n) k_cm /*me*/, uint32_t k_cn /*seed*/);
static uint32_t k_ab(type_(k_n) k_ct /*me*/);
static void k_ac(k_n* k_cv);
static type_(k_n) k_ad(type_(k_n) k_cw);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_cz);
static type_(k_n) k_af(type_(k_n) k_df, type_(Array_<uint8_t>) k_dg, int64_t* k_dh);
static type_(k_n) k_ag(type_(k_n) k_dk /*me2*/, uint32_t k_dl /*seed*/);
using k_dm=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dn /*me_*/, type_(Array_<int64_t>) k_do /*type*/, T_ k_dp /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_ap(type_(Array_<char16_t>) k_dx /*str*/);
template<typename T_>static T_ k_cd(T_ k_dy /*me_*/, type_(Array_<int64_t>) k_dz /*type*/, T_ k_ea /*n*/);
template<typename T_>static T_ k_ce(T_ k_eb /*me_*/, type_(Array_<int64_t>) k_ec /*type*/, int64_t k_ed /*n*/);
template<typename T_>static T_ k_cs(T_ k_eg /*me_*/, type_(Array_<int64_t>) k_eh /*type*/, int64_t k_ei /*n*/);
static type_(Array_<char16_t>) k_dr();
static int64_t k_du(type_(Array_<char16_t>) k_ff /*me_*/, bool* k_fg /*success*/);
static char16_t k_er();
template<typename T_>static T_ k_fe(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/, int64_t k_fx /*start*/, int64_t k_fy /*len*/);
static char16_t k_fl();
...
k_n::k_n(): k_o(), k_co(), k_cp(), k_cq(), k_cr(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap)((((((k_am) % (2LL)) == (0LL)) || (((k_ao) % (2LL)) == (0LL))) ? (newArrayBin_<char16_t>(5,"\x45\x00\x76\x00\x65\x00\x6E\x00\x0A\x00")) : (newArrayBin_<char16_t>(4,"\x4F\x00\x64\x00\x64\x00\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_ay;
type_(k_o) k_az;
(k_ay) = (k_az = new_(k_o)(), k_p(k_az), k_az);
return as_<k_o>(classTable_, (k_ay), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ba /*me*/){
type_(Array_<uint8_t>) k_bb;
(k_bb) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bb;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bc /*me*/, type_(Array_<uint8_t>) k_bd /*bin*/, int64_t* k_be /*idx*/){
type_(k_o) k_bf;
type_(k_o) k_bg;
(k_bf) = (k_bg = new_(k_o)(), k_p(k_bg), k_bg);
return k_bf;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bh /*me*/){
...
(k_bs) = (static_cast<uint64_t>(((k_bn) - (k_bm)) + (1LL)));
(k_bt) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_bs)) % (k_bs)));
(k_bu) = (0x0000000000000000ULL);
if((k_bt) == (0x0000000000000000ULL)){
(k_bu) = ((reinterpret_cast<k_bx>(classTable_[(k_bl)->Y + 9]))((k_bl)));
}
else{
do{
(k_bu) = ((reinterpret_cast<k_bx>(classTable_[(k_bl)->Y + 9]))((k_bl)));
...
return (static_cast<int64_t>((k_bu) % (k_bs))) + (k_bm);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ca /*me*/){
uint32_t k_cb; // t
(k_cb) = ((reinterpret_cast<k_cc>(classTable_[(k_ca)->Y + 12]))((k_ca)));
return k_cd<uint64_t>((k_ce<uint64_t>((static_cast<uint64_t>(k_cb)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cc>(classTable_[(k_ca)->Y + 12]))((k_ca)))));
}
// rndFloat
static double k_z(type_(k_n) k_cf /*me*/, double k_cg /*min*/, double k_ch /*max*/){
if((k_cg) >= (k_ch)){
...
return (((static_cast<double>((reinterpret_cast<k_bx>(classTable_[(k_cf)->Y + 9]))((k_cf)))) / (1.844674407370955e+19)) * ((k_ch) - (k_cg))) + (k_cg);
}
// setSeed
static void k_aa(type_(k_n) k_cm /*me*/, uint32_t k_cn /*seed*/){
((k_cm)->k_co) = (0x075BCD15U);
((k_cm)->k_cp) = (0x159A55E5U);
((k_cm)->k_cq) = ((0x1F123BB5U) * (k_cn));
((k_cm)->k_cr) = ((0x05491333U) * (k_cs<uint32_t>(((k_cm)->k_cq), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_ct /*me*/){
uint32_t k_cu; // t
(k_cu) = (k_ai<uint32_t>(((k_ct)->k_co), (nullptr), (k_ce<uint32_t>(((k_ct)->k_co), (nullptr), (11LL)))));
((k_ct)->k_co) = ((k_ct)->k_cp);
((k_ct)->k_cp) = ((k_ct)->k_cq);
((k_ct)->k_cq) = ((k_ct)->k_cr);
((k_ct)->k_cr) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_ct)->k_cr), (nullptr), (k_cs<uint32_t>(((k_ct)->k_cr), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_cu), (nullptr), (k_cs<uint32_t>((k_cu), (nullptr), (8LL)))))));
return (k_ct)->k_cr;
}
// _dtor
static void k_ac(k_n* k_cv){
...
type_(k_n) k_cx;
type_(k_n) k_cy;
(k_cx) = (k_cy = new_(k_n)(), k_w(k_cy), k_cy);
((k_cx)->k_co) = ((k_cw)->k_co);
((k_cx)->k_cp) = ((k_cw)->k_cp);
((k_cx)->k_cq) = ((k_cw)->k_cq);
((k_cx)->k_cr) = ((k_cw)->k_cr);
return as_<k_n>(classTable_, (k_cx), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_cz){
type_(Array_<uint8_t>) k_da;
(k_da) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_da) = ((k_da)->Cat(toBin_<uint32_t>()(((k_cz)->k_co))));
(k_da) = ((k_da)->Cat(toBin_<uint32_t>()(((k_cz)->k_cp))));
(k_da) = ((k_da)->Cat(toBin_<uint32_t>()(((k_cz)->k_cq))));
(k_da) = ((k_da)->Cat(toBin_<uint32_t>()(((k_cz)->k_cr))));
return k_da;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_df, type_(Array_<uint8_t>) k_dg, int64_t* k_dh){
...
((k_di)->k_co) = (fromBin_<uint32_t>()((k_dg), (*k_dh)));
((k_di)->k_cp) = (fromBin_<uint32_t>()((k_dg), (*k_dh)));
((k_di)->k_cq) = (fromBin_<uint32_t>()((k_dg), (*k_dh)));
((k_di)->k_cr) = (fromBin_<uint32_t>()((k_dg), (*k_dh)));
return k_di;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_dk /*me2*/, uint32_t k_dl /*seed*/){
(reinterpret_cast<k_dm>(classTable_[(k_dk)->Y + 11]))((k_dk), (k_dl));
return k_dk;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dn /*me_*/, type_(Array_<int64_t>) k_do /*type*/, T_ k_dp /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_dx->B),stdout);
#else
std::u16string s_=k_dx->B;
...
// _shl
template<typename T_>static T_ k_ce(T_ k_eb /*me_*/, type_(Array_<int64_t>) k_ec /*type*/, int64_t k_ed /*n*/){
if(k_ed<0||k_ed>=sizeof(k_eb)*8)throw 0xe9170006;
return k_eb<<k_ed;
}
// _shr
template<typename T_>static T_ k_cs(T_ k_eg /*me_*/, type_(Array_<int64_t>) k_eh /*type*/, int64_t k_ei /*n*/){
if(k_ei<0||k_ei>=sizeof(k_eg)*8)throw 0xe9170006;
return k_eg>>k_ei;
}
// inputStr
static type_(Array_<char16_t>) k_dr(){
type_(Array_<char16_t>) k_el; // buf
int64_t k_em; // ptr
char16_t k_eq; // c
(k_el) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_em) = (0LL);
while(true){
(k_eq) = ((k_er)());
if((k_eq) == (u'\uFFFF')){
...
if((k_em) == ((k_el)->Len())){
(k_el) = ((k_el)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_el)->At(k_em)) = (k_eq);
(k_em) = ((k_em) + (1LL));
}
return k_fe<type_(Array_<char16_t>)>((k_el), (nullptr), (0LL), (k_em));
}
// _toInt
static int64_t k_du(type_(Array_<char16_t>) k_ff /*me_*/, bool* k_fg /*success*/){
std::u16string u_=k_ff->B;
...
while(true){
(k_fk) = ((k_fl)());
for(k_fp = (0LL), k_fq = (((k_al)->Len()) - (1LL)); k_fp <= k_fq; k_fp += (1LL)){
if(((k_fk) == ((k_al)->At(k_fp))) || (((k_fk) == (u'\u000D')) && (((k_al)->At(k_fp)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_fk) == (u'\u000D')){
