...
static void k_i();
static void k_j();
static void k_l(int64_t k_bk /*excpt*/);
static void k_p(type_(k_o) k_bn /*me*/);
static void k_q(k_o* k_bo);
static int64_t k_r(type_(k_o) k_bp /*me*/, type_(k_o) k_bq /*t*/);
static type_(k_o) k_s(type_(k_o) k_br /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bu /*me*/);
static type_(k_o) k_u(type_(k_o) k_bw /*me*/, type_(Array_<uint8_t>) k_bx /*bin*/, int64_t* k_by /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_cb /*me*/);
static void k_w(type_(k_n) k_cc /*me*/);
using k_ce=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cf /*me*/, int64_t k_cg /*min*/, int64_t k_ch /*max*/);
...
static int64_t k_ak();
static int64_t k_an();
template<typename T_>static void k_au(T_ k_er /*me_*/, type_(Array_<int64_t>) k_es /*type*/);
template<typename T_>static void k_av(T_ k_et /*me_*/, type_(Array_<int64_t>) k_eu /*type*/);
static void k_bi(type_(Array_<char16_t>) k_ev /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bj(T_ k_ew /*me_*/, type_(Array_<int64_t>) k_ex /*type*/);
template<typename T_>static T_ k_cx(T_ k_ey /*me_*/, type_(Array_<int64_t>) k_ez /*type*/, T_ k_fa /*n*/);
template<typename T_>static T_ k_cy(T_ k_fb /*me_*/, type_(Array_<int64_t>) k_fc /*type*/, int64_t k_fd /*n*/);
template<typename T_>static T_ k_dm(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, int64_t k_fi /*n*/);
static type_(Array_<char16_t>) k_el();
static int64_t k_eo(type_(Array_<char16_t>) k_gf /*me_*/, bool* k_gg /*success*/);
static char16_t k_fr();
template<typename T_>static T_ k_ge(T_ k_gv /*me_*/, type_(Array_<int64_t>) k_gw /*type*/, int64_t k_gx /*start*/, int64_t k_gy /*len*/);
static char16_t k_gl();
...
int64_t k_bc;
(k_am) = ((k_an)());
(k_ao) = (newArrays_<type_(Array_<int64_t>)>((k_am)));
for(k_as = (0LL), k_at = ((k_am) - (1LL)); k_as <= k_at; k_as += (1LL)){
((k_ao)->At(k_as)) = ((k_an)());
}
k_au<type_(Array_<int64_t>)>((k_ao), (nullptr));
k_av<type_(Array_<int64_t>)>((k_ao), (nullptr));
(k_aw) = (0LL);
(k_ax) = (0LL);
for(k_bb = (0LL), k_bc = ((k_am) - (1LL)); k_bb <= k_bc; k_bb += (1LL)){
if(((k_bb) % (2LL)) == (0LL)){
(k_aw) = ((k_aw) + ((k_ao)->At(k_bb)));
}
else{
(k_ax) = ((k_ax) + ((k_ao)->At(k_bb)));
}
}
(k_bi)((((newArrayBin_<char16_t>(0,""))->Cat(k_bj<int64_t>(((k_aw) - (k_ax)), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bs), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bu /*me*/){
type_(Array_<uint8_t>) k_bv;
(k_bv) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bv;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bw /*me*/, type_(Array_<uint8_t>) k_bx /*bin*/, int64_t* k_by /*idx*/){
type_(k_o) k_bz;
type_(k_o) k_ca;
(k_bz) = (k_ca = new_(k_o)(), k_p(k_ca), k_ca);
return k_bz;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_cb /*me*/){
...
return (static_cast<int64_t>((k_co) % (k_cm))) + (k_cg);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cu /*me*/){
uint32_t k_cv; // t
...
static uint32_t k_ab(type_(k_n) k_dn /*me*/){
uint32_t k_do; // t
(k_do) = (k_ai<uint32_t>(((k_dn)->k_di), (nullptr), (k_cy<uint32_t>(((k_dn)->k_di), (nullptr), (11LL)))));
((k_dn)->k_di) = ((k_dn)->k_dj);
((k_dn)->k_dj) = ((k_dn)->k_dk);
((k_dn)->k_dk) = ((k_dn)->k_dl);
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ev->B),stdout);
#else
std::u16string s_=k_ev->B;
...
// _shl
template<typename T_>static T_ k_cy(T_ k_fb /*me_*/, type_(Array_<int64_t>) k_fc /*type*/, int64_t k_fd /*n*/){
if(k_fd<0||k_fd>=sizeof(k_fb)*8)throw 0xe9170006;
return k_fb<<k_fd;
}
// _shr
template<typename T_>static T_ k_dm(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, int64_t k_fi /*n*/){
if(k_fi<0||k_fi>=sizeof(k_fg)*8)throw 0xe9170006;
return k_fg>>k_fi;
}
// inputStr
static type_(Array_<char16_t>) k_el(){
type_(Array_<char16_t>) k_fl; // buf
int64_t k_fm; // ptr
char16_t k_fq; // c
(k_fl) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fm) = (0LL);
while(true){
(k_fq) = ((k_fr)());
if((k_fq) == (u'\uFFFF')){
...
if((k_fm) == ((k_fl)->Len())){
(k_fl) = ((k_fl)->Cat(newArrays_<type_(Array_<char16_t>)>((1024LL))));
}
((k_fl)->At(k_fm)) = (k_fq);
(k_fm) = ((k_fm) + (1LL));
}
return k_ge<type_(Array_<char16_t>)>((k_fl), (nullptr), (0LL), (k_fm));
}
// _toInt
static int64_t k_eo(type_(Array_<char16_t>) k_gf /*me_*/, bool* k_gg /*success*/){
std::u16string u_=k_gf->B;
...
while(true){
(k_gk) = ((k_gl)());
for(k_gp = (0LL), k_gq = (((k_al)->Len()) - (1LL)); k_gp <= k_gq; k_gp += (1LL)){
if(((k_gk) == ((k_al)->At(k_gp))) || (((k_gk) == (u'\u000D')) && (((k_al)->At(k_gp)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gk) == (u'\u000D')){
